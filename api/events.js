...
// api-src/events.ts
var SWAP_FEE_APPLIED_EVENT = parseAbiItem(
...
async function handler(req, res) {
  ...
  const limit = Math.min(100, parseInt(url.searchParams.get("limit") ?? req.query?.limit ?? "20", 10));
  ...
  try {
    ...
    const logs = await client.getLogs({ address: hookAddr, event: SWAP_FEE_APPLIED_EVENT, fromBlock, toBlock: "latest" });
    ...
    const events = sliced.map((log) => ({
      ...
      appliedFeePips: Number(log.args.appliedFee ?? 3e3),
      appliedFeeBps: Number(log.args.appliedFee ?? 3e3) / 100,
    ...
    const computedToxicShareBps = totalEvents > 0 ? Math.round(toxicEvents / totalEvents * 1e4) : 0;
    res.statusCode = 200;
