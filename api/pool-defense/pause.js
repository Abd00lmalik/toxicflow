...
// api-src/pool-defense/pause.ts
var ABI = parseAbi(["function pauseSwaps(bytes32 reason) external"]);
async function handler(req, res) {
  ...
  if (!apiKey || auth !== `Bearer ${apiKey}`) {
    res.statusCode = 401;
    res.end(JSON.stringify({ paused: false, error: "Unauthorized" }));
    return;
  }
  ...
  try {
    ...
    const reasonBytes32 = `0x${buf.toString("hex")}`;
    const hash3 = await walletClient.writeContract({ address: hookAddress, abi: ABI, functionName: "pauseSwaps", args: [reasonBytes32] });
    await publicClient.waitForTransactionReceipt({ hash: hash3 });
    res.statusCode = 200;
    res.end(JSON.stringify({ paused: true, txHash: hash3, reason, pausedAt: (/* @__PURE__ */ new Date()).toISOString() }));
  } catch (err) {
