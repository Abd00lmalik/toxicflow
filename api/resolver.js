...
// node_modules/.pnpm/viem@2.48.7_typescript@5.9.3_zod@4.3.6/node_modules/viem/_esm/clients/transports/fallback.js
init_node();
init_rpc();
...
var TIER_LABELS = { 0: "neutral", 1: "trusted", 2: "toxic" };
async function handler(req, res) {
  ...
  const trader = url.searchParams.get("trader") ?? req.query?.trader;
  ...
  if (!registryAddr) {
    res.statusCode = 200;
    res.end(JSON.stringify({ trader, tier: 0, tierLabel: "neutral", hasActivePassport: false, feeBps: 30, feePips: 3e3, resolverType: "not-configured" }));
    return;
  }
  try {
    ...
    const [tier, hasPassport] = await Promise.all([
      client.readContract({ address: registryAddr, abi: PASSPORT_REGISTRY_ABI, functionName: "getTier", args: [trader] }),
      client.readContract({ address: registryAddr, abi: PASSPORT_REGISTRY_ABI, functionName: "hasPassport", args: [trader] })
    ]);
    let feePips = Number(tier) === 1 ? 1e3 : Number(tier) === 2 ? 8e3 : 3e3;
    if (hookAddr) {
      try {
        const [fp] = await client.readContract({ address: hookAddr, abi: HOOK_ABI, functionName: "previewFee", args: [trader] });
        feePips = fp;
      } catch {
      }
    }
